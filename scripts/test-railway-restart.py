...
def main():
 ...
 for phase in ('write','read','verify'):
  ...
  with logfile.open('w') as log:code=run(['./gradlew','--no-daemon','runServer',f'-PciRailwayRestart={phase}',f'-PciServerDir={a.output}'],ROOT,log,300)
