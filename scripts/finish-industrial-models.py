...
PALETTE={
 ...
 'filter_mesh':'iron_bars','wash_window':'cyan_terracotta','steel_casing':'polished_andesite',
...
def frame(material='stone'):
 e=[part([0,0,0],[16,16,16],material)]
...
model('warehouse_controller',frame('wood')+[panel(3,3,10,10,'dark')]+dial(5,7)+[panel(5,4,6,1,'brass',-.25)])
...
for name,metal in [('cargo_loader','copper'),('cargo_unloader','green')]:
 model(name,frame()+[panel(2,2,12,12,'dark'),panel(3,6,10,4,metal),panel(5,4,6,2,metal),panel(5,10,6,2,metal),panel(5,7,6,2,'black',-.25)])
model('bulk_tank',frame('green')+[panel(6,3,4,10,'brass'),panel(7,4,2,8,'blue',-.25),panel(7,10,2,2,'paper',-.375)])
model('market_counter',[part([0,0,0],[16,14,16],'wood'),part([0,14,0],[16,16,16],'wood',up='map'),panel(2,2,12,10,'dark'),panel(3,3,10,8,'wood',-.25),panel(7,8,2,1,'brass',-.375),part([0,0,0],[2,14,16],'iron'),part([14,0,0],[16,14,16],'iron')])
