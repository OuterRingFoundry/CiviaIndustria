...
public final class CarriageChecks {
    public static Train fixture(net.minecraft.server.level.ServerLevel level,BlockPos anchor,long quantity) throws Exception {
        ...
        var cargo=(CargoBlockEntity)level.getBlockEntity(anchor.above());String[] keys=new String[16];Arrays.fill(keys,"");keys[0]="minecraft:iron_ingot";long[] counts=new long[16];counts[0]=quantity;cargo.restoreMounted(keys,counts);
        ...
        try{carriage.setContraption(level,contraption);contraption.removeBlocksFromWorld(level,BlockPos.ZERO);return train;}
        catch(Exception|AssertionError failure){remove(train);throw failure;}
        finally{glue.discard();}
    }
    public static void remove(Train train){train.carriages.forEach(c->c.forEachPresentEntity(entity->entity.discard()));com.simibubi.create.Create.RAILWAYS.removeTrain(train.id);com.simibubi.create.Create.RAILWAYS.removeGraphAndGroup(train.graph);}
    public static void run(GameTestHelper h) throws Exception {
        var level=h.getLevel();var train=fixture(level,h.absolutePos(new BlockPos(2,1,2)),5_000_000_000L);
        var carriage=train.carriages.getFirst();var graph=train.graph;
        try{
