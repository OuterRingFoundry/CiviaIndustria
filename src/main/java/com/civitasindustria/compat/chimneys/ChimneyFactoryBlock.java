...
public final class ChimneyFactoryBlock extends FactoryBlock implements ISmokeEmitter {
    private final Emitter emitter=new Emitter(Emitter.Properties.tile(ACTIVE).smoke(0xff69645b,0.5f).maxGapLength(1));
    public ChimneyFactoryBlock(Properties p){super(p);}
    @Override public Emitter getEmitter(LevelReader level,BlockPos pos){return emitter;}
