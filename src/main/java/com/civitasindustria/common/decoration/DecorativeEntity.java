...
/** Persistent animation/control parameters. Neither side registers a ticker. */
public final class DecorativeEntity extends BlockEntity {
    ...
    public boolean usable(){return quarantine==null&&!isRemoved();}
    public int signal(){return usable()&&enabled?Math.clamp((rpm+3)/4,0,15):0;}
    public void change(boolean speed){if(quarantine!=null)return;if(speed)rpm=rpm>=60?5:rpm+5;else enabled=!enabled;startTime=level.getGameTime();setChanged();level.sendBlockUpdated(worldPosition,getBlockState(),getBlockState(),3);if(DecorativeFunctions.kind(this).equals("decorative_gear"))level.updateNeighborsAt(worldPosition,getBlockState().getBlock());}
